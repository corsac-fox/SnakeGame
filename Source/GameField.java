...
class GameField extends JPanel {
	...
	int bodyCount = 1;
	Vector <SnakeGame.BodyElement> body = new Vector <SnakeGame.BodyElement>();
	...
	public void paintComponent(Graphics g) {
		...
		if (body.size() == 0) {
		    ...
		    body.add(new BodyElement(x, y, Snake.direction));
			x = 60;
			body.add(new BodyElement(x, y, Snake.direction));
		}
	...
	public int scoreCount() {
		if (Point.distance(x, y, foodX, foodY) < 30) {
			...
			isFood = true;
			foodCount++;
			body.add(new BodyElement(foodX, foodY, Snake.direction));
			x = foodX;
			y = foodY;
			isCount = true;						
			bodyCount++;
